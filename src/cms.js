...
function renderList() {
  ...
  mount.innerHTML = `
    ${flashMessage ? `<div class="phase-card cms-flash status-success" role="status">${esc(flashMessage)}</div>` : ''}
  ...
  if (flashMessage) {
    const messageAtRender = flashMessage;
    flashMessage = '';
    window.setTimeout(() => {
      const flash = mount.querySelector('.cms-flash');
      if (flash && flash.textContent === messageAtRender) flash.remove();
    }, 5000);
  }
...
function showForm(item) {
  if (!mount) return;
  if (!item) {
    alert('編集対象の出展社データを取得できませんでした。再読み込みしてください。');
    return;
  }
  mount.innerHTML = form(item);
  mount.scrollIntoView({ behavior: 'smooth', block: 'start' });
  window.setTimeout(() => mount.querySelector('[name="companyName"]')?.focus(), 250);
  ...
  el?.addEventListener('submit', async event => {
    ...
    try {
      ...
      flashMessage = `${body.companyName || '出展社'}の情報を保存しました。公開サイトへ即時反映されます。`;
      await load(false);
    } catch (error) {
